...
RSpec.describe PostHog::MCP::ConversationId do
  ...
  describe '.resolve' do
    it 'echoes a minted-shaped handle (lowercased) and mints over anything else' do
      expect(described_class.resolve(true, handle, 'a_tool', 'get_more_tools')).to eq([handle, false])
      expect(described_class.resolve(true, " #{handle.upcase} ", 'a_tool', 'get_more_tools')).to eq([handle, false])
      %w[conv-1 1 session chat_abc not-a-uuid 019fd2b0-1111-4111-8111-111111111111].each do |value|
        id, minted = described_class.resolve(true, value, 'a_tool', 'get_more_tools')
        expect(minted).to be(true)
        expect(id).not_to eq(value)
        expect(id).to match(described_class::MINTED_CONVERSATION_ID)
      end
      minted_id, = described_class.resolve(true, nil, 'a_tool', 'get_more_tools')
      expect(described_class.resolve(true, minted_id, 'a_tool', 'get_more_tools')).to eq([minted_id, false])
    end

    it 'mints when handed no value it owns' do
      # A tool that declares `conversation_id` itself is never stripped, so nothing
      # reaches `.resolve` and its value cannot anchor an analytics conversation.
      [nil, 123, '   '].each do |supplied|
        id, minted = described_class.resolve(true, supplied, 'a_tool', 'get_more_tools')
        expect(minted).to be(true)
        expect(id).to match(described_class::MINTED_CONVERSATION_ID)
      end
    end
    ...
    it 'is inert when disabled or for the missing-capability tool' do
      expect(described_class.resolve(false, handle, 'a_tool', 'get_more_tools')).to eq([nil, false])
      expect(described_class.resolve(true, nil, 'get_more_tools', 'get_more_tools')).to eq([nil, false])
    end
  end
